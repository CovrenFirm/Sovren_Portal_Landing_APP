...
export default async function handler(
  ...
  let endpoint = '/api/crm/companies';

  if (id && typeof id === 'string') {
    endpoint = `/api/crm/companies/${id}`;
  }
  ...
  switch (method) {
    case 'GET':
      // GET /api/crm/companies or GET /api/crm/companies/:id
    ...
    case 'PUT':
      // PUT /api/crm/companies/:id - Full update
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Company ID is required for PUT requests'
        });
      }
      if (!req.body) {
        return res.status(400).json({
    ...
    case 'PATCH':
      // PATCH /api/crm/companies/:id - Partial update
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Company ID is required for PATCH requests'
        });
      }
      if (!req.body) {
        return res.status(400).json({
    ...
    case 'DELETE':
      ...
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Company ID is required for DELETE requests'
        });
      }
