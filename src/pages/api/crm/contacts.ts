...
export default async function handler(
  ...
  switch (method) {
    case 'GET':
      // GET /api/crm/contacts or GET /api/crm/contacts/:id
    ...
    case 'PUT':
      ...
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Contact ID is required for PUT requests'
        });
      }
      if (!req.body) {
        return res.status(400).json({
    ...
    case 'PATCH':
      ...
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Contact ID is required for PATCH requests'
        });
      }
      if (!req.body) {
        return res.status(400).json({
    ...
    case 'DELETE':
      ...
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Contact ID is required for DELETE requests'
        });
      }
