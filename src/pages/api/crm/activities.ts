...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<ApiResponse<Activity | Activity[]>>
) {
  const { method, query } = req;
  const { id } = query;

  let endpoint = '/api/crm/activities';

  if (id && typeof id === 'string') {
    endpoint = `/api/crm/activities/${id}`;
  }
  ...
  switch (method) {
    case 'GET':
      // GET /api/crm/activities or GET /api/crm/activities/:id
    ...
    case 'PUT':
      // PUT /api/crm/activities/:id - Full update
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Activity ID is required for PUT requests'
        });
      }
      if (!req.body) {
        return res.status(400).json({
    ...
    case 'PATCH':
      ...
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Activity ID is required for PATCH requests'
        });
      }
      if (!req.body) {
        return res.status(400).json({
    ...
    case 'DELETE':
      ...
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Activity ID is required for DELETE requests'
        });
      }
