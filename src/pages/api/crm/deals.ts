...
export default async function handler(
  ...
  let endpoint = '/api/crm/deals';

  if (id && typeof id === 'string') {
    endpoint = `/api/crm/deals/${id}`;
  }
  ...
  switch (method) {
    case 'GET':
      // GET /api/crm/deals or GET /api/crm/deals/:id
    ...
    case 'PUT':
      // PUT /api/crm/deals/:id - Full update
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Deal ID is required for PUT requests'
        });
      }
      if (!req.body) {
        return res.status(400).json({
    ...
    case 'PATCH':
      ...
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Deal ID is required for PATCH requests'
        });
      }
      if (!req.body) {
        return res.status(400).json({
    ...
    case 'DELETE':
      ...
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Deal ID is required for DELETE requests'
        });
      }
