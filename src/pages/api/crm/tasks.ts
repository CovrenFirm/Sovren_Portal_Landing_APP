...
export default async function handler(
  ...
  switch (method) {
    case 'GET':
      // GET /api/crm/tasks or GET /api/crm/tasks/:id
    ...
    case 'PUT':
      // PUT /api/crm/tasks/:id - Full update
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Task ID is required for PUT requests'
        });
      }
      if (!req.body) {
        return res.status(400).json({
    ...
    case 'PATCH':
      // PATCH /api/crm/tasks/:id - Partial update
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Task ID is required for PATCH requests'
        });
      }
      if (!req.body) {
        return res.status(400).json({
    ...
    case 'DELETE':
      ...
      if (!id) {
        return res.status(400).json({
          success: false,
          error: 'Task ID is required for DELETE requests'
        });
      }
