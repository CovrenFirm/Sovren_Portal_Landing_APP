...
export default async function handler(
  ...
  try {
    ...
    if (req.query.executiveId) queryParams.append("executiveId", String(req.query.executiveId));
    ...
    if (req.query.action) queryParams.append("action", String(req.query.action));

    const queryString = queryParams.toString();
    const url = `${EXECUTIVE_SERVICE_URL}/executives/activity${queryString ? `?${queryString}` : ""}`;
    ...
    if (!response.ok) {
      return res.status(response.status).json({
        ...
        message: data.message || "Failed to fetch executive activity",
