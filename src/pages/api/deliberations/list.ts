...
interface DeliberationListResponse {
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<DeliberationListResponse | ErrorResponse>
) {
  ...
  try {
    ...
    if (req.query.participant) queryParams.append("participant", String(req.query.participant));
    ...
    const url = `${EXECUTIVE_SERVICE_URL}/deliberations/list${queryString ? `?${queryString}` : ""}`;
    ...
    if (!response.ok) {
      return res.status(response.status).json({
        ...
        message: data.message || "Failed to fetch deliberations list",
